...
exports.auth = function(req, res) {
    ...
    if(!password || !email)
        return res.status(422).send(createErrorObject('Data Missing','Email và password là bắt buộc!'));
    ...
    User.findOne({email}, function(error, user){
        ...
        if(!user)
            return res.status(422).send(createErrorObject('Invalid user','Người dùng không tồn tại!'));
        ...
        if(user.isSamePassword(password)){
            ...
            return res.status(422).send(createErrorObject('Wrong Detail','Người dùng không tồn tại hoặc sai mật khẩu'));
        }
    });
...
exports.register = function(req, res) {
    ...
    if(!password || !email){
        return res.status(422).send(createErrorObject('Data Missing','Email và mật khẩu không được để trống!'));
    }

    // if both password not match than throw this error 
    if(password !== passwordConfirm){
        return res.status(422).send(createErrorObject('Invalid password','Mật khẩu phải trùng với mật khẩu xác nhận!'));
    }
    ...
    User.findOne({email}, (error, user) => {
        ...
        if(user)
            return res.status(422).send(createErrorObject('Invalid Email',`Người dùng với email ${email} đã tồn tại!`));
